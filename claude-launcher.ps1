...
if (-not (Get-Command cligate -ErrorAction SilentlyContinue)) {
    ...
    Write-Host "✓ CliGate is installed." -ForegroundColor Green
}

# 3. Check if CliGate is already running on port 8081
$portInUse = netstat -ano | Select-String ":8081" | Select-String "LISTENING"
if ($portInUse) {
    Write-Host "✓ CliGate is already running on port 8081." -ForegroundColor Green
} else {
    ...
    if ($checkPort) {
        Write-Host "✓ CliGate started successfully." -ForegroundColor Green
    } else {
...
Write-Host "✓ Environment variables configured for local proxy." -ForegroundColor Green
