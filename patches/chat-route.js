...
import { sendAntigravityMessage, isAntigravityModel, toPublicAntigravityModel } from '../antigravity-api.js';
...
export async function handleChatCompletion(req, res) {
  ...
  const hasAntigravityAccounts = listAntigravityAccounts().total > 0;
  ...
  if (hasAntigravityAccounts && isAntigravityModel(requestedModel)) {
    const result = await _handleViaAntigravityAccount(res, body, requestedModel, upstreamModel, startTime);
...
async function _handleChatAssignment(req, res, body, requestedModel, upstreamModel, startTime, assignment) {
  ...
  for (const candidate of assignments) {
    ...
    if (candidate.credentialType === 'antigravity-account') {
      const result = await _handleViaAssignedAntigravityAccount(res, body, requestedModel, upstreamModel, startTime, candidate.credential.email);
...
async function _handleViaAntigravityAccount(res, body, requestedModel, upstreamModel, startTime, preferredEmail = null) {
  ...
  for (const account of accounts) {
    ...
    try {
      const anthropicRequest = _buildAnthropicRequest(body, upstreamModel);
      logger.request('POST', '/v1/chat/completions', { model: upstreamModel, account: account.email, messages: body.messages?.length || 0 });
      const response = await sendAntigravityMessage(anthropicRequest, account, { modelOverride: requestedModel });
      const duration = Date.now() - startTime;
      logger.response(200, { model: upstreamModel, tokens: response.usage?.output_tokens || 0, duration });
      ...
      logger.error(`Failed to handle chat via antigravity account ${account.email}: ${error.message}`);
...
async function _handleViaAssignedAntigravityAccount(res, body, requestedModel, upstreamModel, startTime, email) {
  return _handleViaAntigravityAccount(res, body, requestedModel, upstreamModel, startTime, email);
}
